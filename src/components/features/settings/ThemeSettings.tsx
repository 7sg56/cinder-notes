...
export function ThemeSettings() {
    ...
    const [colorMode, setColorMode] = useState<'light' | 'dark' | 'system'>('dark'); // Simplified for now
        ...
        <div className="flex-1 overflow-y-auto w-full h-full p-8 no-scrollbar bg-[var(--bg-primary)]">
            <div className="max-w-5xl mx-auto space-y-10">
                ...
                <div className="space-y-4">
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">Choose if app's appearance should be light or dark, or follow your computer's settings.</p>
                    ...
                    <div className="flex gap-4">
                        {[
                            ...
                            >
                                ...
                                <span className={colorMode === mode.id ? 'font-medium text-[var(--text-primary)]' : 'text-[var(--text-secondary)]'}>
                                    {mode.label}
                                </span>
                            </button>
                        ))}
                ...
                <div className="space-y-4 pt-4">
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">Choose a preset theme from our library.</p>
                        ...
                        {THEME_PRESETS.map((theme) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${isActive
                ...
                <div className="space-y-6 pt-8 border-t border-[var(--border-primary)]">
                    <div className="flex justify-between items-center">
                        <div>
                            <h2 className="text-lg font-semibold text-[var(--text-primary)]">Custom themes</h2>
                            <p className="text-sm text-[var(--text-secondary)]">Set your own app theme by changing the color for each interface element.</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                            ...
                            <div key={idx} className="flex items-center justify-between p-4 bg-[var(--bg-secondary)] rounded-lg border border-[var(--border-primary)]">
                                <div>
                                    <div className="text-xs font-medium text-[var(--text-primary)] mb-1">{item.label}</div>
                                    <div className="text-xs text-[var(--text-tertiary)] font-mono">{item.hex}</div>
                                </div>
